...
.box1 {
	...
	<div style="margin-top: 300px; margin-left: 520px;">
		<span><a style="color: #000;"
			href="movieController?op=searchMovie&title=${title}&pageNum=1">首页</a></span>
		<c:if test="${pageBean.havePrePage }">
			<span><a style="color: #000;"
				href="movieController?op=searchMovie&title=${title}&pageNum=${pageBean.pageNum-1}">上一页</a></span>
		</c:if>
		<span>第${pageBean.pageNum}页</span>
		<form action="movieController?op=searchMovie&title=${title}"
		...
		<c:if test="${pageBean.haveNextPage}">
			<span><a style="color: #000;"
				href="movieController?op=searchMovie&title=${title}&pageNum=${pageBean.pageNum+1}">下一页</a></span>
		</c:if>
		<span><a style="color: #000;"
			href="movieController?op=searchMovie&title=${title}&pageNum=${pageBean.totalPages}">最后一页</a></span>
	</div>
